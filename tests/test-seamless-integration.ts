...
async function testSeamlessIntegration() {
    ...
    const { data: githubRelease, error: githubError } =
        await sharedGitHubClient.getLatestRelease();
    ...
    const { data: latestVersion } =
        await sharedVersionManager.getLatestVersion();
    if (latestVersion) {
    ...
    console.log(
        "🎯 All shared utilities work identically across server and client"
    );
}
