...
export async function exampleLoggingUsage() {
    ...
    const userActionLogs = await loggingController.getLogsByEventType(
        "user_action",
        20
    );
...
export async function exampleSceneUsage() {
    ...
    if (newScene.success && newScene.data?.id) {
        const likeResult = await sceneController.likeScene(
            "user456",
            newScene.data.id
        );
...
export async function exampleErrorHandling() {
    try {
        ...
        await loggingController.logError(error as Error, "server", {
