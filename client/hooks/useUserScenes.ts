...
class UserScenesService {
  ...
  async updateScene(id: string, input: UserSceneUpdateInput): Promise<UserScene> {
    await this.delay();
...
export const useUserScenes = () => {
  ...
  const saveScene = useCallback(async (input: UserSceneCreateInput): Promise<UserScene> => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
  ...
  const updateScene = useCallback(async (id: string, input: UserSceneUpdateInput): Promise<UserScene> => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
    
    try {
      const updatedScene = await userScenesService.updateScene(id, input);
      
      setState(prev => ({ 
        ...prev, 
        scenes: prev.scenes.map(s => s.id === id ? updatedScene : s),
        isLoading: false
      }));
  ...
  const duplicateScene = useCallback(async (id: string, newName: string): Promise<UserScene> => {
    try {
  ...
  const importScene = useCallback(async (jsonData: string): Promise<UserScene> => {
    try {
