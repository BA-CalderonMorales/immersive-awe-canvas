...
export const useSceneSettingsViewModel = (
  ...
  }, [currentBackground, currentGeometry, selectedBackgroundId, selectedGeometryId]);
  ...
  const handleBackgroundChange = useCallback((backgroundId: string) => {
    ...
    const backgroundIndex = backgrounds?.findIndex(bg => bg.id === parseInt(backgroundId));
  ...
  const handleGeometryChange = useCallback((geometryId: string) => {
    setSelectedGeometryId(geometryId);
    const geometryIndex = geometries?.findIndex(geo => geo.id === parseInt(geometryId));
