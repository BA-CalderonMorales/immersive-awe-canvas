...
type SettingsAction = 
  ...
  | { type: 'UPDATE_MATERIAL_PROPERTY'; payload: { property: string; value: string | number | boolean } }
  | { type: 'UPDATE_LIGHT_PROPERTY'; payload: { lightIndex: number; property: string; value: string | number | boolean | number[] } }
  | { type: 'UPDATE_BACKGROUND_PROPERTY'; payload: { property: string; value: string | number | boolean } }
  | { type: 'RESET_SCENE' }
  | { type: 'TOGGLE_MOTION' };
...
export const useSettingsPanelViewModel = ({
  ...
  const dispatchSceneUpdate = useCallback((action: SettingsAction) => {
  ...
  const updateMainObjectColor = useCallback((color: string) => {
  ...
  const updateMaterialType = useCallback((materialType: string) => {
    updateMaterialProperty('materialType', materialType);
  }, [updateMaterialProperty]);

  const updateMaterialMetalness = useCallback((metalness: number) => {
    updateMaterialProperty('metalness', metalness);
  }, [updateMaterialProperty]);

  const updateMaterialRoughness = useCallback((roughness: number) => {
  ...
  const updateAmbientLightIntensity = useCallback((intensity: number) => {
    ...
    const ambientIndex = sceneConfig[theme].lights.findIndex(light => light.type === 'ambient');
    if (ambientIndex !== -1) {
      updateLightProperty(ambientIndex, 'intensity', intensity);
    }
  }, [updateLightProperty, sceneConfig, theme]);

  const updateDirectionalLightIntensity = useCallback((intensity: number) => {
    // Find directional light index
    const directionalIndex = sceneConfig[theme].lights.findIndex(light => light.type === 'directional');
  ...
  const updateBackgroundType = useCallback((backgroundType: string) => {
