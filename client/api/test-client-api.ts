...
async function testClientAPI() {
    ...
    const { data: releases, error: releasesError } =
        await clientGitHubAPIClient.getReleases(3);
    if (releases) {
        console.log(
    ...
    // Test Logging API
    console.log("\n📝 Testing Logging API:");
    const { data: logResult, error: logError } =
    ...
    const { data: migrationVersion } =
        await migration.version.fetchLatestRelease();
...
if (typeof window !== "undefined") {
    ...
    console.log(
        "🚀 Client API test function available at: window.testClientAPI()"
    );
}
