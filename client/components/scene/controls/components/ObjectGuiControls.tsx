...
const ObjectGuiControls = ({ object, onUpdate }: ObjectGuiControlsProps) => {
  ...
  useEffect(() => {
    ...
    controllers.positionX = positionFolder.add(positionProxy, 'x', -10, 10, 0.01).name('X Position').onChange((value: number) => {
      console.log('🎯 GUI Position X changed:', value, 'proxy:', positionProxy);
    ...
    controllers.positionY = positionFolder.add(positionProxy, 'y', -10, 10, 0.01).name('Y Position').onChange((value: number) => {
    ...
    controllers.positionZ = positionFolder.add(positionProxy, 'z', -10, 10, 0.01).name('Z Position').onChange((value: number) => {
    ...
    controllers.rotationX = rotationFolder.add(rotationProxy, 'x', 0, Math.PI * 2, 0.01).name('X Rotation').onChange((value: number) => {
    ...
    controllers.rotationY = rotationFolder.add(rotationProxy, 'y', 0, Math.PI * 2, 0.01).name('Y Rotation').onChange((value: number) => {
    ...
    controllers.rotationZ = rotationFolder.add(rotationProxy, 'z', 0, Math.PI * 2, 0.01).name('Z Rotation').onChange((value: number) => {
    ...
    controllers.scale = scaleFolder.add(uniformScale, 'value', 0.1, 5, 0.01).name('Uniform Scale').onChange((value: number) => {
      // CRITICAL FIX: Only pass the specific property update to avoid stale object references
      onUpdate({ 
    ...
    controllers.color = materialFolder.addColor(colorProxy, 'color').name('Object Color').onChange((value: string) => {
    ...
    if (object.material.metalness !== undefined) {
      controllers.metalness = materialFolder.add(materialProxy, 'metalness', 0, 1, 0.01).name('Metalness').onChange((value: number) => {
    ...
    if (object.material.roughness !== undefined) {
      controllers.roughness = materialFolder.add(materialProxy, 'roughness', 0, 1, 0.01).name('Roughness').onChange((value: number) => {
    ...
    controllers.opacity = materialFolder.add(materialProxy, 'opacity', 0, 1, 0.01).name('Opacity').onChange((value: number) => {
      materialProxy.opacity = value;
      // CRITICAL FIX: Only pass the specific property update to avoid stale object references
      onUpdate({ 
        material: { ...materialProxy, opacity: value, transparent: value < 1 } 
      });
    });
  ...
  useEffect(() => {
    ...
    object.color,
    object.material.metalness, object.material.roughness, object.material.opacity,
    object.material.wireframe, object.material.transparent
  ]);
