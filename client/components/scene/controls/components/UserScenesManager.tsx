...
const UserScenesManager = ({ onLoadScene, trigger }: UserScenesManagerProps) => {
  ...
  const filteredScenes = scenes.filter(scene => {
    ...
    const matchesTag = selectedTag === null || scene.tags.includes(selectedTag);
    
    return matchesSearch && matchesTag;
  });

  // Get all unique tags
  const allTags = Array.from(new Set(scenes.flatMap(scene => scene.tags))).sort();
  ...
  const handleDuplicateScene = async (scene: UserScene) => {
    const newName = prompt('Enter a name for the duplicated scene:', `${scene.name} (Copy)`);
                      ...
                      <div className="flex items-center justify-between text-sm text-muted-foreground">
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-1">
                            ...
                            {formatDistanceToNow(new Date(scene.createdAt), { addSuffix: true })}
                          </div>
