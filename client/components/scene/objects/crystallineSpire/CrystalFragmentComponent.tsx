...
const CrystalFragmentComponent = ({
    fragment,
    color,
    materialConfig,
    onRef,
    ...
    const clusterColors = [
        color,
        color,
        `hsl(${((parseInt(color.slice(1), 16) % 360) + 20) % 360}, 70%, 60%)`,
        `hsl(${((parseInt(color.slice(1), 16) % 360) + 40) % 360}, 70%, 55%)`,
    ];
    ...
    return (
        <group
            ...
            scale={[
                fragment.scale,
                fragment.scale * (1.2 + Math.sin(fragment.phaseOffset) * 0.3),
                fragment.scale,
            ]}
        >
