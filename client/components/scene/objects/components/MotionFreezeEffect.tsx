...
const MotionFreezeEffect = ({ isMotionFrozen = false, targetRef, children }: MotionFreezeEffectProps) => {
  ...
  useFrame(() => {
    if (isMotionFrozen && targetRef.current) {
      ...
      if (!obj.userData?.isBeingDragged) {
        obj.position.set(frozen.position.x, frozen.position.y, frozen.position.z);
        obj.rotation.set(frozen.rotation.x, frozen.rotation.y, frozen.rotation.z);
