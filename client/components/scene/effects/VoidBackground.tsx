...
const VoidBackground = ({ theme }: VoidBackgroundProps) => {
  ...
  useFrame((state) => {
    ...
    fragments.forEach((fragment, i) => {
      ...
      const orbital = Math.sin(timeRef.current * 0.1 + fragment.phase) * 5;
      fragment.position.add(fragment.velocity);
      fragment.position.x += Math.sin(timeRef.current * 0.05 + fragment.phase) * 0.01;
      fragment.position.y += Math.cos(timeRef.current * 0.03 + fragment.phase * 2) * 0.008;
      ...
      const energyPulse = 1 + Math.sin(timeRef.current * 0.5 + fragment.phase) * 0.3;
      const distanceScale = 1 + (fragment.position.length() / 100) * 0.2;
      const pulsingScale = fragment.scale * energyPulse * distanceScale * (0.5 + fragment.energy * 0.5);
      
      dummy.current.position.copy(fragment.position);
      dummy.current.scale.setScalar(pulsingScale);
      dummy.current.rotation.x += fragment.rotationSpeed * (1 + fragment.energy);
      dummy.current.rotation.y += fragment.rotationSpeed * 0.7 * (1 + fragment.energy);
      ...
      const timeColor = Math.sin(timeRef.current * 0.2 + fragment.phase) * 0.5 + 0.5;
      ...
      if (theme === 'day') {
        ...
        const lightness = 0.7 + energy * 0.2 + Math.sin(timeRef.current + fragment.phase) * 0.1;
        ...
        const lightness = 0.2 + energy * 0.6 + Math.sin(timeRef.current * 0.3 + fragment.phase) * 0.2;
        color = new Color().setHSL(hue, saturation, lightness);
      }
