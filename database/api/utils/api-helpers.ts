...
export async function withRetry<T>(
    ...
    const { maxAttempts, delay, backoffFactor } = { ...DEFAULT_RETRY_CONFIG, ...config };
    let lastError: Error;
 ...
 */
export function formatErrorResponse(error: Error | APIError, context?: Record<string, any>) {
    const response = {
        error: error.message,
...
export async function logApiOperation<T>(
    ...
    try {
        ...
        await databaseLoggingClient.logError(
            ...
            { duration: `${duration}ms`, error: errorMessage, success: false, ...context }
        );
