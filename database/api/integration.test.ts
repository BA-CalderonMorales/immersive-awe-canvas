...
describe('Database API Integration', () => {
    ...
    describe('Clients', () => {
        it('should create edge function client', async () => {
            const { SupabaseEdgeFunctionClient } = await import('../clients/edge-function-client.js');
        ...
        it('should create REST client', async () => {
            const { SupabaseRestClient } = await import('../clients/supabase-rest-client.js');
        ...
        it('should create logging client', async () => {
            const { DatabaseLoggingClient } = await import('../clients/logging-client.js');
        ...
        it('should create GitHub integration client', async () => {
            const { GitHubIntegrationClient } = await import('../clients/github-integration-client.js');
    ...
    describe('Validation', () => {
        it('should validate issue data correctly', async () => {
            const { validateIssueData } = await import('../utils/validation.js');
    ...
    describe('Error Handling', () => {
        it('should create database errors correctly', async () => {
            const { createDatabaseError, DatabaseErrorType } = await import('../utils/error-handling.js');
        ...
        it('should map status codes to error types', async () => {
            const { mapStatusToErrorType, DatabaseErrorType } = await import('../utils/error-handling.js');
            
            expect(mapStatusToErrorType(400)).toBe(DatabaseErrorType.VALIDATION_ERROR);
            expect(mapStatusToErrorType(401)).toBe(DatabaseErrorType.AUTHENTICATION_ERROR);
            expect(mapStatusToErrorType(404)).toBe(DatabaseErrorType.NOT_FOUND_ERROR);
            expect(mapStatusToErrorType(500)).toBe(DatabaseErrorType.SERVER_ERROR);
        });
    });

    describe('API Helpers', () => {
        it('should sanitize input data', async () => {
            const { sanitizeInputData } = await import('../utils/api-helpers.js');
        ...
        it('should build query parameters', async () => {
            const { buildQueryParams } = await import('../utils/api-helpers.js');
    ...
    describe('Integration Flow', () => {
        it('should handle complete issue creation flow', async () => {
            ...
            expect(typeof supabaseEdgeFunctionClient.createGithubIssue).toBe('function');
        });
    });
});
