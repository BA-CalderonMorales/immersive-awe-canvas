...
serve(async req => {
    ...
    try {
        ...
        // Basic validation
        if (
            !sanitizedIssueData.issueLocation ||
            sanitizedIssueData.issueLocation.length < 3
        ) {
            return sendResponse(
                { error: "Issue location must be at least 3 characters" },
                400
            );
        }
        ...
        if (
            ...
            sanitizedIssueData.expectedBehavior.length < 10
        ) {
            return sendResponse(
                { error: "Expected behavior must be at least 10 characters" },
                400
            );
        }
...
`;
        ...
        if (!response.ok) {
            ...
            if (response.status === 404) {
                errorDetail =
                    "Repository not found. Ensure GITHUB_REPO is correct and your token has access.";
            } else if (response.status === 401) {
                errorDetail =
                    "Authentication failed. Check your GITHUB_ACCESS_TOKEN.";
            } else {
                errorDetail =
