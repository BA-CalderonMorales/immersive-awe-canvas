...
const EnergyStreams = ({ materialConfig }: EnergyStreamsProps) => {
  ...
  return (
    <>
      ...
      {[...Array(4)].map((_, i) => { // Reduced to 4 for deeper focus
        ...
        return (
          <mesh 
            ...
            rotation={[timeRef.current * 0.2 + i, timeRef.current * 0.15, timeRef.current * 0.1]} // Slow, contemplative rotation
            scale={[0.06, 2.0 + Math.sin(timeRef.current * 0.6 + i) * 0.4, 0.06]} // Breathing length
          >
            ...
            <DynamicMaterial 
              materialConfig={{
                ...materialConfig,
                transparent: true,
                opacity: 0.3 + Math.sin(timeRef.current * 0.5 + i) * 0.1, // Pulsing visibility
      ...
      {[...Array(8)].map((_, i) => {
        ...
        return (
          <mesh
            ...
            position={[
              radius * Math.sin(phi) * Math.cos(theta + timeRef.current * 0.03),
              radius * Math.cos(phi) + Math.sin(timeRef.current * 0.2 + i) * 0.5,
              radius * Math.sin(phi) * Math.sin(theta + timeRef.current * 0.03)
            ]}
            rotation={[
          ...
          >
            ...
            <DynamicMaterial
              materialConfig={{
                ...materialConfig,
                transparent: true,
                opacity: 0.25 + Math.sin(timeRef.current * 0.8 + i) * 0.1,
