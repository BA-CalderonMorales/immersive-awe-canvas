...
export class LightControlsBuilder {
  ...
  private createLightControls(parentFolder: GUI, light: LightConfig, index: number) {
    const lightFolder = parentFolder.addFolder(`Light ${index + 1} (${light.type})`);
  ...
  private createPositionControls(parentFolder: GUI, light: LightConfig, index: number) {
    const posFolder = parentFolder.addFolder('Position');
    
    posFolder.add(light.position!, '0', -200, 200).name('x').onChange(v => 
      this.updateConfig(c => { 
        c[this.theme].lights[index].position![0] = v; 
      })
    );

    posFolder.add(light.position!, '1', -200, 200).name('y').onChange(v => 
      this.updateConfig(c => { 
        c[this.theme].lights[index].position![1] = v; 
      })
    );

    posFolder.add(light.position!, '2', -200, 200).name('z').onChange(v => 
      this.updateConfig(c => { 
